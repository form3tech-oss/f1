...
func NewActiveScenarios(testRun string, env map[string]string, fn MultiStageSetupFn, autoTeardownIdleDuration time.Duration) (*ActiveScenario, error) {
...
func (s *ActiveScenario) Run(metric metrics.MetricType, stage, vu, iter string, f func(t *T)) error {
	t := NewT(s.env, vu, iter, s.TestRun)
	start := time.Now()
	...
	// wait for completion
	<-done
	s.m.Record(metric, s.TestRun, stage, metrics.Result(t.HasFailed()), time.Since(start).Nanoseconds())
	if t.HasFailed() {
...
func (s *ActiveScenario) RecordDroppedIteration() {
	s.m.Record(metrics.IterationResult, s.TestRun, "single", "dropped", 0)
}
