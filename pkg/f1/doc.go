...
Test scenarios consist of two stages:
 ...
 1. The test setup which is called once at the start of a test. This may be useful for generating
    resources needed for all tests, or subscribing to message queues.

 2. A single iteration's run function. This is called for every iteration of the test, often in
    parallel with other iterations.
...
Writing tests is simply a case of implementing the types and registering them with F1:
	...
	func setupMySuperFastLoadTest(t *testing.T) testing.RunFn {
		...
		// execution to clean up the setup
		t.Cleanup(func() {
...
Once you have written a load test and compiled a binary test runner, you can use the various
"trigger modes" (See here for more details:
https://github.com/form3tech-oss/f1/tree/master/internal/trigger) that F1 supports. These are
available as subcommands to the "f1 run" command, so trying "f1 run --help" will provide more
information. The trigger modes currently implemented are as follows:

- constant: applies load at a constant rate (e.g. one request per second, irrespective of request
duration).

- staged: applies load at various stages (e.g. one request per second for 10s, then two per second
for 10s).

- users: applies load from a pool of users (e.g. requests from two users being sent sequentially -
they are as fast or as slow as the requests themselves).

- gaussian: applies load based on a Gaussian distribution (e.g. varies load throughout a given
duration with a mean and standard deviation).

- ramp: applies load constantly increasing or decreasing an initial load during a given ramp
duration (e.g. from 0/s requests to 100/s requests during 10s).

- file: applies load based on a yaml config file - the file can contain any of the previous load
modes.
